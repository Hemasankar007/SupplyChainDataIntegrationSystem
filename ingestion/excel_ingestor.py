...
class ExcelConnector:
    ...
    def __init__(self):
        self.logger = record_pipeline_event("ExcelConnector", "STARTED")
    ...
    def _validate_orders_data(self, df):
        ...
        if missing_pct > 0.05:  # 5% threshold
            record_data_validation("Orders Missing Data", "WARNING", f"Missing data: {missing_pct:.2%}")
        else:
            record_data_validation("Orders Missing Data", "PASS", f"Missing data: {missing_pct:.2%}")
        
        # Check for required columns
        required_columns = ['Order ID', 'Order Date', 'Ship Date', 'Customer ID', 'Product ID']
        missing_columns = [col for col in required_columns if col not in df.columns]
        if missing_columns:
            record_data_validation("Orders Required Columns", "FAIL", f"Missing columns: {missing_columns}")
        else:
            record_data_validation("Orders Required Columns", "PASS")
    ...
    def _validate_returns_data(self, df):
        ...
        missing_pct = df.isnull().sum().sum() / (df.shape[0] * df.shape[1])
        if missing_pct > 0.05:
            record_data_validation("Returns Missing Data", "WARNING", f"Missing data: {missing_pct:.2%}")
        else:
            record_data_validation("Returns Missing Data", "PASS", f"Missing data: {missing_pct:.2%}")
    ...
    def _validate_people_data(self, df):
        ...
        missing_pct = df.isnull().sum().sum() / (df.shape[0] * df.shape[1])
        if missing_pct > 0.05:
            record_data_validation("People Missing Data", "WARNING", f"Missing data: {missing_pct:.2%}")
        else:
            record_data_validation("People Missing Data", "PASS", f"Missing data: {missing_pct:.2%}")
